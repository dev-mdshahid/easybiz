alter table public.business_settings
  add column if not exists screenshot_batch_size integer not null default 16;

alter table public.business_settings
  drop constraint if exists business_settings_screenshot_batch_size_chk;

alter table public.business_settings
  add constraint business_settings_screenshot_batch_size_chk
  check (
    screenshot_batch_size >= 1
    and screenshot_batch_size <= 16
  );

drop function if exists public.commit_expected_order_intake(bigint, bigint, text, jsonb, jsonb);
...
revoke all on function public.commit_expected_order_intake(
  bigint,
  bigint,
  text,
  jsonb,
...
grant execute on function public.commit_expected_order_intake(
  bigint,
  bigint,
  text,
  jsonb,
