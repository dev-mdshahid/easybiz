...
alter table public.businesses
  add constraint businesses_opening_balance_pair_chk
    check (
      (opening_balance is null and opening_balance_on is null)
      or (opening_balance is not null and opening_balance_on is not null)
    ),
  add constraint businesses_opening_balance_nonneg_chk
...
as $$
  ...
  payouts as (
    ...
    where i.business_id = p_business_id
      ...
      and (timezone('Asia/Dhaka', i.created_at))::date > biz.opening_balance_on
  )
  select case
